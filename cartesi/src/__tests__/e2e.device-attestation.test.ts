 ...
 */
/**
 * Per-submission salt folded into the signed hash (H-2). Fixed here so hashes are
 * reproducible; real devices generate 16 random bytes per submission.
 */
 ...
 *
 * The salt is what stops an observer brute-forcing low-entropy sensor readings out
 * of the public on-chain hash; did and timestamp are both public. Cartesi verifies
 * the JWS over whatever hash it is given, so omitting the salt here would let this
 * suite pass while exercising a device protocol that no longer exists.
 */
function computeDataHash(
  ...
  const combined = canonical + deviceDid + String(timestamp) + salt;

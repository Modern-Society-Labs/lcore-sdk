...
export class AttestorSocket implements IAttestorSocket {
	...
	constructor(
		...
		// Type assertions needed due to ws/DOM WebSocket type incompatibility
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		const socketAny = socket as any

		socketAny.addEventListener('error', (event: ErrorEvent) => {
		...
		socketAny.addEventListener('close', () => (
		...
		socketAny.addEventListener('message', async({ data }: { data: unknown }) => {
			try {

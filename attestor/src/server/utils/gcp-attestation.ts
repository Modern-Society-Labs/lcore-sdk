...
export async function validateGcpAttestationAndExtractKey(
	...
	try {
		...
		// Audience can be:
		// 1. Custom Locale audience with data param: https://locale.cash/attestation?data=tee_k_public_key:0x...
		// 2. Locale domain only: https://locale.cash (address in eat_nonce)
		// 3. GCP STS audience: https://sts.googleapis.com (for Confidential Space)
		const hasLocaleAudience = payload.aud?.includes('locale.cash')
		const hasGcpStsAudience = payload.aud?.includes('sts.googleapis.com')

		if(!hasLocaleAudience && !hasGcpStsAudience) {
			errors.push(`Invalid audience: expected "locale.cash" or "sts.googleapis.com", got "${payload.aud}"`)
		}
